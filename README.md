...
| openvpn_port                       | `1194`                                            | The port you want OpenVPN to run on. If you have different ports on   different servers, I suggest you set the port in your inventory file.                     |
...
| openvpn_config_file                | `openvpn_{{ openvpn\_proto }}_{{ openvpn_port }}` | The config file name you want to   use                                                                                                                          |
...
| openvpn_uninstall                  | `false`                                           | Set to true to uninstall the OpenVPN service                                                                                                                    |
...
| openvpn_verify_cn                  | `false`                                           | Check that the CN of the certificate match the FQDN                                                                                                             |
| openvpn_redirect_gateway           | `true`                                            | OpenVPN gateway push                                                                                                                                            |
| openvpn_set_dns                    | `true`                                            | Will push DNS to the client (Google and OpenDNS)                                                                                                                |
...
| openvpn_ca_key                     |                                                   | Contain "crt" and "key". If not set, CA cert and key   will be automatically generated on the target system.                                                    |
...
| openvpn_ldap                       |                                                   | Dictionary that contain LDAP configuration                                                                                                                      |
...
| openvpn_bridge_dev                 | `br0`                                             | Name of the bridge interface beeing used for bridgeing                                                                                                          |
...
| tls_ca_cert_file    | `/etc/openvpn/auth/ca.pem`                | Path to the CA ldap backend. This must must has been pushed before                             |
